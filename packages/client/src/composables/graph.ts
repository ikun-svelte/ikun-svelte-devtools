...
export const setGraphConfigCache = (config: IGraphConfig) => localCache.setJSON(svelteGraphConfig, config)
...
export const initGraphData = (search: string, data) => {
...
export function getHoverPath(level: 'custom' | 'absolute' | 'root', fullPath: string, rootPath: string, levelCustom) {
  switch (level) {
    case 'absolute':
      return fullPath
    case 'custom':

...
import { parseRoute, DIR_CLIENT, createRPCServer } from "@ikun-svelte-devtools/utils-server"
...
import {
  ...
  V_OPTIONS,
  V_PATH_APP_LOAD_KIT,
} from '@ikun-svelte-devtools/shared'
function getSvelteDevtoolsPath() {
  const pluginPath = normalizePath(path.dirname(fileURLToPath(import.meta.url)))
  return pluginPath.replace(/\/dist$/, '//src/server')
}
...
export default function VitePluginSvelteDevTools(options: VitePluginSvelteDevToolsOptions): PluginOption {
  ...
  const hookPath = svelteDevtoolsPath.replace('server', 'utils')
  return [
    skRouterGuard({

...
import type { AssetInfo, AssetType, ImageMeta } from '@ikun-svelte-devtools/shared';
...
function guessType(path: string): AssetType {
	...
	if (/\.(json[5c]?|te?xt|[mc]?[jt]sx?|md[cx]?|markdown)/i.test(path)) return 'text';
	return 'other';
}
	...
	return await Promise.all(
		files.map(async (path) => {
			...
			const normalizedPath =
				publicDirname === p.basename(p.dirname(path)) ? path.replace(publicDirname, '') : path;
			return {
				path: normalizedPath,
				filePath,
